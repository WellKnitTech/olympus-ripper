...
def verify_self_integrity(quiet: bool = False) -> bool:
    """
    ...
    if not manifest_path.exists():
        ...
        if not quiet:
            print(f"  {_d}[INTEGRITY] No manifest found — generating baseline...{_RST}")
        save_manifest()
        if not quiet:
            print(f"  {_G}[INTEGRITY] Baseline manifest created.{_RST}")
        return True
    ...
    try:
        ...
        if not quiet:
            print(f"  {_Y}[INTEGRITY] WARNING: Manifest file corrupt — regenerating.{_RST}")
        save_manifest()
        return True
    ...
    if not modified and not missing:
        if not quiet:
            print(f"  {_G}[INTEGRITY] All core files verified — SHA-256 match.{_RST}")
        return True
